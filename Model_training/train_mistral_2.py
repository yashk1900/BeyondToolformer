...
Your task is to generate a chain of abstractions (C) for the given question (Q) using the available tools: Wiki and QA tool. You have to use combination of tools to derive the answer (C). Follow the rules and formats provided for each tool:                               
**Tools:** 
1. **Wiki Tool:** Retrieves relevant articles from Wikipedia. * **Format:** `[search query -Wiki-> search query output]` 
2. **QA Tool:** Extracts focused answers from Wikipedia articles. * **Format:** `[input context -QA(question)-> output]` 
...
train=pd.read_pickle("sep_data/wiki_train.pkl")
eval=pd.read_pickle("sep_data/wiki_val.pkl")
...
args=transformers.TrainingArguments(
    ...
    eval_steps=40,               # Evaluate and save checkpoints every 100 steps
